...
test("readAppConfig loads conventional conf files from the working directory", () => {
  ...
  writeFileSync(
    ...
    [
      "CLIO_FS_CLIENT_CONTROL_PLANE_BASE_URL=http://sync.example.internal:7010",
  ...
  assert.equal(config.server.port, 7010);
  ...
  assert.equal(config.client.controlPlaneBaseUrl, "http://sync.example.internal:7010");
