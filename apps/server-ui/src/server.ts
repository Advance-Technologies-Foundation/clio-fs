...
const resolveConfFilePath = (name: string): string => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (existsSync(example)) return candidate;
...
const renderServerConfigButton = () => `
  <button type="button" class="topbar-button" onclick="serverOpenConfigDialog()">Config</button>
  <dialog id="server-config-dialog" class="dialog" style="width:80vw;height:80vh;max-width:80vw;max-height:80vh;margin:auto;padding:0;border:none;border-radius:16px;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,0.18);">
    <div style="width:100%;height:100%;display:flex;flex-direction:column;background:#fff;border-radius:16px;overflow:hidden;">
      <div class="modal-header" style="flex-shrink:0;">
        ...
        <button class="modal-close" type="button" onclick="serverConfigCancel()" aria-label="Close">×</button>
      </div>
      <div class="modal-body" style="flex:1;overflow:hidden;display:flex;flex-direction:column;padding-bottom:0;">
        <p id="server-config-file-path" style="margin:0 0 0.5rem;font-size:0.78rem;color:#64748b;font-family:monospace;word-break:break-all;flex-shrink:0;"></p>
        <div id="server-config-load-error" style="display:none;color:#b91c1c;margin-bottom:0.5rem;font-size:0.85rem;flex-shrink:0;"></div>
        <textarea id="server-config-textarea" style="flex:1;width:100%;font-family:monospace;font-size:0.84rem;line-height:1.55;border:1px solid #e2e8f0;border-radius:8px;padding:0.75rem;resize:none;box-sizing:border-box;background:#f8fafc;overflow:auto;white-space:pre;word-wrap:normal;overflow-wrap:normal;" spellcheck="false"></textarea>
        <div id="server-config-save-status" style="margin-top:0.45rem;font-size:0.78rem;min-height:1.2em;flex-shrink:0;padding-bottom:0.25rem;"></div>
      </div>
      <div class="modal-actions" style="flex-shrink:0;">
        <button class="secondary-button" type="button" onclick="serverConfigCancel()">Cancel</button>
        <button class="primary-button" type="button" onclick="serverSaveConfig()">Save</button>
      </div>
    </div>
  </dialog>
  <dialog id="server-config-discard-dialog" class="dialog">
    <div class="modal-card">
      <div class="modal-header">
        ...
        <button class="modal-close" type="button" onclick="document.getElementById('server-config-discard-dialog').close()" aria-label="Close">×</button>
      </div>
      <div class="modal-body">
        <p class="lede" style="margin-top:0;">You have unsaved changes. Discard them and close the editor?</p>
      </div>
      <div class="modal-actions">
        <button class="secondary-button" type="button" onclick="document.getElementById('server-config-discard-dialog').close()">Keep editing</button>
        <button class="danger-button" type="button" onclick="serverDiscardAndClose()">Discard</button>
      </div>
    </div>
  </dialog>
  <script>
    var _serverOriginalConfig = '';
    function serverOpenConfigDialog() {
      var dialog = document.getElementById('server-config-dialog');
      var textarea = document.getElementById('server-config-textarea');
      var filePath = document.getElementById('server-config-file-path');
      var loadError = document.getElementById('server-config-load-error');
      var saveStatus = document.getElementById('server-config-save-status');
      _serverOriginalConfig = '';
      textarea.value = '';
      ...
      fetch('/config').then(function(r){ return r.json(); }).then(function(data) {
        ...
        _serverOriginalConfig = textarea.value;
    ...
    function serverConfigCancel() {
      var textarea = document.getElementById('server-config-textarea');
      if (textarea.value !== _serverOriginalConfig) {
        document.getElementById('server-config-discard-dialog').showModal();
        return;
      }
      document.getElementById('server-config-dialog').close();
    }
    function serverDiscardAndClose() {
      document.getElementById('server-config-discard-dialog').close();
      document.getElementById('server-config-dialog').close();
    }
...
const renderServerRestartButton = () => `
  <button type="button" class="secondary-button" onclick="document.getElementById('server-restart-confirm-dialog').showModal()">Restart</button>
  <dialog id="server-restart-confirm-dialog" class="dialog">
    <div class="modal-card">
      <div class="modal-header">
        <div>
          <p class="table-card-label" style="margin-bottom:0.35rem;">Confirm</p>
        ...
        <button class="modal-close" type="button" onclick="document.getElementById('server-restart-confirm-dialog').close()" aria-label="Close">×</button>
      </div>
      <div class="modal-body">
        <p class="lede" style="margin-top:0;">Restart the server UI process? The page will reload automatically.</p>
      </div>
      <div class="modal-actions">
        <button class="secondary-button" type="button" onclick="document.getElementById('server-restart-confirm-dialog').close()">Cancel</button>
        <button class="primary-button" type="button" onclick="serverRestartProcess()">Restart</button>
      </div>
    </div>
  </dialog>
  <script>
    function serverRestartProcess() {
      document.getElementById('server-restart-confirm-dialog').close();
...
const renderServerTopbarActions = () =>
  `${renderHomeLink()}${renderAdminLink()}${renderLogsLink()}${renderAboutLink()}${renderServerSettingsButton()}${renderLogoutButton()}${renderServerConfigButton()}${renderServerRestartButton()}${renderServerShutdownButton()}`;
  ...
  return async (request: IncomingMessage, response: ServerResponse) => {
    try {
      ...
      if (method === "GET" && url.pathname === "/config") {
        const configPath = resolveConfFilePath("server.conf");
